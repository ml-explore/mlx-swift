...
public class OptimizerBase<State> : Optimizer {
    ...
    final func apply(gradients: ModuleParameters, model: Module) -> ModuleParameters {
        let (p, s) = gradients.mapValues(model.parameters(), state) { gradient, parameter, state in
            ...
            applySingle(gradient: gradient, parameter: parameter!, state: state ?? newState(gradient: gradient))
        }
        self.state = s
        return p
    }
...
public class SGD : OptimizerBaseArrayState {
    ...
    public init(learningRate: Float, momentum: Float = 0, weightDecay: Float = 0, dampening: Float = 0, nesterov: Bool = false) {
    ...
    override func applySingle(gradient: MLXArray, parameter: MLXArray, state: MLXArray) -> (MLXArray, MLXArray) {
        if momentum <= 0 {
...
public class AdaDelta : OptimizerBase<(MLXArray, MLXArray)> {
    ...
    override func applySingle(gradient: MLXArray, parameter: MLXArray, state: State) -> (MLXArray, State) {
        var v = state.0
        var u = state.1
...
public class AdamW : Adam {
    ...
    public init(learningRate: Float, betas: (Float, Float) = (0.9, 0.999), eps: Float = 1e-8, weightDecay: Float = 0.01) {
        self.weightDecay = weightDecay
        super.init(learningRate: learningRate, betas: betas, eps: eps)
    }

    override func applySingle(gradient: MLXArray, parameter: MLXArray, state: Adam.State) -> (MLXArray, Adam.State) {
        super.applySingle(gradient: gradient, parameter: parameter * (1 - learningRate * weightDecay), state: state)
    }
}
...
public class Adafactor : OptimizerBase<Adafactor.State> {
    ...
    public init(learningRate: Float? = nil, eps: (Float, Float) = (1e-30, 1e-3), clipThreshold: Float = 1, decayRate: Float = -0.8, beta1: Float? = nil, weightDecay: Float = 0, scaleParameter: Bool = true, relativeStep: Bool = true, warmupInit: Bool = false) {
    ...
    override func applySingle(gradient: MLXArray, parameter: MLXArray, state: State) -> (MLXArray, State) {
        ...
        if factored {
            var expAvgSqRow = state.expAvgSqRow ?? MLXArray.zeros(gradient.shape.dropLast(), dtype: gradient.dtype)
            var expAvgSqCol = state.expAvgSqCol ?? MLXArray.zeros(Array(gradient.shape.dropFirst()), dtype: gradient.dtype)
