...
public class MultiHeadAttention: Module {
    ...
    public func callAsFunction(
        ...
        queries = queries.reshape(B, S, numHeads, -1).transpose(0, 2, 1, 3)
        ...
        let valuesHat = matmul(scores, values).transpose(0, 2, 1, 3).reshape(B, L, -1)
    ...
    public static func createAdditiveCausalMask(_ n: Int, dtype: DType = .float32) -> MLXArray {
        ...
        var mask = indices.reshape(-1, 1) < indices.reshape(1, -1)
