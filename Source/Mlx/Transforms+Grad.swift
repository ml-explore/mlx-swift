...
///
///
public func grad(_ f: @escaping ([MLXArray]) -> [MLXArray], argumentNumbers: [Int] = [0]) -> (
    [MLXArray]
) -> [MLXArray] {
...
/// See ``grad(_:)-7zhu6``
public func grad(_ f: @escaping ([MLXArray]) -> (MLXArray, MLXArray), argumentNumbers: [Int] = [0])
...
/// See ``grad(_:)-7zhu6``
public func grad(
    _ f: @escaping ([MLXArray]) -> (MLXArray, MLXArray, MLXArray), argumentNumbers: [Int] = [0]
...
/// See ``grad(_:)-7zhu6``
public func grad(_ f: @escaping (MLXArray) -> (MLXArray, MLXArray, MLXArray)) -> (MLXArray) -> (
    MLXArray, MLXArray, MLXArray
) {
    let wrappedFunction = wrapResult(wrapArguments(f))
    let gradientFunction = buildValueAndGradient(wrappedFunction, argumentNumbers: [0])
...
@inline(__always)
private func wrapArguments<Result>(_ f: @escaping (MLXArray, MLXArray) -> Result) -> ([MLXArray]) ->
    Result
{
    { (arrays: [MLXArray]) in
        f(arrays[0], arrays[1])
...
@inline(__always)
private func wrapArguments<Result>(_ f: @escaping (MLXArray, MLXArray, MLXArray) -> Result) -> (
    [MLXArray]
) -> Result {
    { (arrays: [MLXArray]) in
...
@inline(__always)
private func unwrapArguments(_ f: @escaping ([MLXArray]) -> [MLXArray]) -> (
    MLXArray, MLXArray, MLXArray
) -> [MLXArray] {
...
@inline(__always)
private func unwrapResult(_ f: @escaping ([MLXArray]) -> [MLXArray]) -> ([MLXArray]) -> (
    MLXArray, MLXArray
) {
    { (a0: [MLXArray]) in
        let v = f(a0)
