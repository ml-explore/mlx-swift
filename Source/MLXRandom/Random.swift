...
/// ```
public func uniform<R: HasDType, T>(_ range: Range<R>, _ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
...
/// let array = MLXRandom.uniform(0.5 ..< 1, [50], key: key)
/// ```
public func uniform<T>(_ range: Range<Float> = 0 ..< 1, _ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
    let lb = MLXArray(range.lowerBound)
    let ub = MLXArray(range.upperBound)
    return MLXArray(mlx_random_uniform(lb.ctx, ub.ctx, shape.asInt32, shape.count, T.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func uniform<T>(low: ScalarOrArray, high: ScalarOrArray, _ shape: [Int]? = nil, type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
    let (low, high) = toArrays(low, high)
    let shape = shape ?? low.shape
    return MLXArray(mlx_random_uniform(low.ctx, high.ctx, shape.asInt32, shape.count, T.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func normal<T>(_ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
...
/// ```
public func randInt<T>(_ range: Range<T>, _ shape: [Int] = [], key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryInteger {
    let lb = MLXArray(range.lowerBound)
    let ub = MLXArray(range.upperBound)
...
/// ```
public func randInt(low: ScalarOrArray, high: ScalarOrArray, _ shape: [Int]? = nil, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray {
    let (low, high) = toArrays(low, high)
    let shape = shape ?? low.shape
    return MLXArray(mlx_random_randint(low.ctx, high.ctx, shape.asInt32, shape.count, low.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func randInt<T>(low: ScalarOrArray, high: ScalarOrArray, _ shape: [Int]? = nil, type: T.Type, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryInteger {
    let (low, high) = toArrays(low, high)
    let shape = shape ?? low.shape
    return MLXArray(mlx_random_randint(low.ctx, high.ctx, shape.asInt32, shape.count, T.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func bernoulli(_ shape: [Int] = [], key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray {
    let p = MLXArray(0.5)
...
/// ```
public func bernoulli(_ p: ScalarOrArray, _ shape: [Int]? = nil, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray {
...
public func truncatedNormal<R: HasDType, T>(_ range: Range<R>, _ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
    let lb = MLXArray(range.lowerBound)
    let ub = MLXArray(range.upperBound)
...
/// let array = MLXRandom.truncatedNormal(0.5 ..< 1, [50], key: key)
/// ```
public func truncatedNormal<T>(_ range: Range<Float>, _ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
    let lb = MLXArray(range.lowerBound)
    let ub = MLXArray(range.upperBound)
    return MLXArray(mlx_random_truncated_normal(lb.ctx, ub.ctx, shape.asInt32, shape.count, T.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func truncatedNormal<T>(low: ScalarOrArray, high: ScalarOrArray, _ shape: [Int]? = nil, type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
    let (low, high) = toArrays(low, high)
    let shape = shape ?? low.shape
    return MLXArray(mlx_random_truncated_normal(low.ctx, high.ctx, shape.asInt32, shape.count, T.dtype.cmlxDtype, key?.ctx, stream.ctx))
}
...
/// ```
public func gumbel<T>(_ shape: [Int] = [], type: T.Type = Float.self, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray where T: HasDType, T: BinaryFloatingPoint {
...
public func categorical(_ logits: MLXArray, axis: Int = -1, shape: [Int]? = nil, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray {
    if let shape {
...
/// - Parameters:
///     - logits: The *unnormalized* categorical distribution(s).
public func categorical(_ logits: MLXArray, axis: Int = -1, count: Int, key: MLXArray? = nil, stream: StreamOrDevice = .default) -> MLXArray {
